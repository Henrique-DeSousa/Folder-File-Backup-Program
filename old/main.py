...
def user_input(folder_path, log_directory, sync_interval):
    while True:
    ...
    while True:
        try:
            ...
            print("Please insert a valid interval.")
    minutes = sync_interval * 3
    if folder_path == log_directory:
        log_setup(os.path.dirname(folder_path))
        backup_folder_path = create_backup_folder(folder_path)
        eventloop.create_task(sync(minutes, folder_path, backup_folder_path))
    else:
...
def get_folder_name(path_user):  # P:\Projects\Python\Folder and File Backup Program\folder_test
    ...
    for files in os.listdir(target_path):
        ...
        if os.path.isdir(source_folder):
            ...
            if not os.path.exists(destination_folder):
                ...
                with open(source_folder, "rb+") as f1, open(destination_folder,
                                                            "rb+") as f2:  # opens files with read bytes
                    hash_source_file = hashlib.md5(
                        f1.read(buffer_size)).hexdigest()  # generates the hash md5 of the file
                    ...
                    if hash_source_file != hash_backup_file:  # compares both hashes and checks if there's a change
                        ...
                        log_write(
                            f"Modified File: {files} in {destination_folder}")  # writes to the log file the changes
